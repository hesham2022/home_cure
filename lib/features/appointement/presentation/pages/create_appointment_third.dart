...
import 'package:home_cure/features/appointement/presentation/blocs/check_timeslot_cubit/cubit.dart';
...
class _CreateAppointementThirdPageState
  ...
  late CheckTimeslotsCubit checkTimeslotsCubit;
  @override
  void initState() {
    _period = context.read<AppointmentsParamsCubit>().state.period!;
    checkTimeslotsCubit = CheckTimeslotsCubit(getIt());
    if (context.read<AppointmentsParamsCubit>().state.date != null) {
      checkTimeslotsCubit.checkTimeSlotFunc(
        context.read<AppointmentsParamsCubit>().state.date!.toString(),
      );
    }
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider<CheckTimeslotsCubit>.value(
      value: checkTimeslotsCubit,
      child: SingleChildScrollView(
        child: BlocBuilder<CheckTimeslotsCubit, CheckTimeslotsCubitState>(
          builder: (context, checkTimeslotsCubitState) {
            if (checkTimeslotsCubitState is CheckTimeslotsCubitStateLoaded) {
              return BlocBuilder<TimeSlotCubit, TimeSlotCubitState>(
                builder: (context, state) {
                  if (state is TimeSlotCubitStateLoaded) {
                    ...
                    final befor = checkTimeslotsCubitState.appointments
                        .map((e) => e.timeslot.id)
                        .toList();
                    timeSlosts = [...timeSlosts]
                        .where((e) => !befor.contains(e.id))
                        .toList();
                    print(befor);
                    return Column(
                      children: [
                          ...
                          child: timeSlosts.isEmpty
                              ? Column(
                                  ...
                                  children: [
                                    Center(
                                      child: Text(
                                        context.l10n.notAvaialableAtThatDy,
                                      ),
                                    ),
                                  ],
                                )
            ...
            if (checkTimeslotsCubitState is CheckTimeslotsCubitStateLoading) {
              return SizedBox(
            ...
            if (checkTimeslotsCubitState is CheckTimeslotsCubitStateError) {
              return Center(
                child: Text(checkTimeslotsCubitState.error.errorMessege),
              );
            }
            return const SizedBox();
          },
...
class TimeSlotListWisdegt extends StatelessWidget {
  const TimeSlotListWisdegt({super.key});
